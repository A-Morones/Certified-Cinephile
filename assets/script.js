...
//Fetch Rotten Tomatoes Score//
const fetchRottenTomatoesScore = function(movieTitle) {
  fetch(`${OMDB_URL}?apikey=${OMDB_API_KEY}&t=${(movieTitle)}&tomatoes=true&r=json`)
    ...
    .then(data => {
      if (data.Ratings) {
        ...
        if (rtRating) {
          ...
          const rtScoreElement = document.createElement("div");
          rtScoreElement.innerHTML = "Rotten Tomatoes Score: Not Available";
          document.getElementById('movie-details').appendChild(rtScoreElement);
        }
      } else {
